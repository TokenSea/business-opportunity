...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  if (auth.user.role !== "ADMIN") {
    return NextResponse.json({ message: "仅管理员可以修改合同" }, { status: 403 });
  }
  try {
    const { id } = await context.params;
    const input = linkedRecordUpdateSchema.parse(await request.json());
    const row = await prisma.$transaction(async (tx) => {
      await tx.contract.update({ where: { id }, data: { name: input.name, notes: input.notes || null } });
      await tx.auditLog.create({
        data: { userId: auth.user.id, action: "UPDATE", entityType: "CONTRACT", entityId: id },
      });
